...
public class ArticleController extends BaseController {
	...
	public static final String ID_SUFFIX = ".html";
    ...
    public String page(@PathParam String cid, Request request) {
       	String _cidString = cid;
    	if (_cidString.endsWith(ArticleController.ID_SUFFIX)) {
    		_cidString = _cidString.replace(ArticleController.ID_SUFFIX, "");
		}
        Contents contents = contentsService.getContents(_cidString);
        if (null == contents) {
     ...
     * 文章页
     */
    @GetRoute(value = {"article/:cid", "article/:cid.html" , "article/:cid/:cp.html"})
    public String post(Request request, @PathParam String cid,@PathParam(name = "cp", defaultValue = "1") String cp) {
        Contents contents = contentsService.getContents(cid.replace(ArticleController.ID_SUFFIX, ""));
        if (null == contents) {
            return this.render_404();
        }
        ...
        if (contents.getAllowComment()) {
            request.attribute("cp", Integer.valueOf(cp.replace(ArticleController.ID_SUFFIX, "")));
        }
        Contents temp = new Contents();
        temp.setHits(contents.getHits() + 1);
        temp.updateById(contents.getCid());
     ...
     * 浏览次数操作
     */
    @GetRoute(value = "/statistic/hits/:cid")
    @JSON
    public RestResponse<?> statisticHits(Request request,@HeaderParam String Referer, @PathParam String cid) {
        if (StringKit.isBlank(Referer)) {
            return RestResponse.fail(ErrorCode.BAD_REQUEST);
        }
        if (!Referer.startsWith(Commons.site_url())) {
        ...
        Contents contents = contentsService.getId(cid);
        String  val   = request.address() + ":" + cid;
        Integer count = cache.hget(Types.ARTICLE_HITS_HITS, val);
        if (null != count && count > 0) {
            return RestResponse.ok(contents.getHits());
        }
        contentsService.updateHists(cid);
        try {
            // 设置对每个文章30秒可以评论一次
            cache.hset(Types.ARTICLE_HITS_HITS, val, 1, 30);
            return RestResponse.ok(contents.getHits() + 1);
        } catch (Exception e) {
            String msg = "更新浏览次数失败";
            if (e instanceof ValidatorException) {
                msg = e.getMessage();
