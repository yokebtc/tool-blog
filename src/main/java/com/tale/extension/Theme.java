...
//        }
     ...
     * @return
     */
    public static List<Contents> get_nav_list() {
    	List<Contents>  listAnimaQuery = cache.get("get_nav_list");
    	if (listAnimaQuery == null) {
    		listAnimaQuery = select().from(Contents.class)
    				.select("cid,title,slug,slug_title,slug_order,thumb_img")
    				.where(Contents::getType, Types.PAGE)
    				.notEq(Contents::getSlugTitle, "NULL")
    				.and("status", Types.PUBLISH)
    				.order(Contents::getSlugOrder, OrderBy.DESC)
    				.all();
    		cache.set("get_nav_list", listAnimaQuery, 60 * 60 * 30);
		}
        return listAnimaQuery;
    }
